...
export const POST = async (req: NextRequest) => {
  ...
  try {
    const newResponsable = await prisma.responsableSante.create({
      data: {
    ...
    return NextResponse.json({
      message: "Error creating responsable",
      error: error,
      status: 400,
    });
  }
...
export const GET = async (req: NextRequest) => {
  ...
  try {
    if (id) {
      const responsable = await prisma.responsableSante.findUnique({
        where: { id: Number(id) },
        include: {
          sante: true, // Inclusion du responsable
        },
      });
      if (!responsable) {
        return NextResponse.json({
    ...
    const responsables = await prisma.responsableSante.findMany({
      include: {
        sante: true, // Inclusion du responsable
      },
    });
...
export const DELETE = async (req: NextRequest) => {
  ...
  try {
    ...
    await prisma.responsableSante.delete({
      where: { id: Number(id) },
    });
    ...
    return NextResponse.json({
      message: "Error deleting responsable" + error,
      error: error,
      status: 400,
    });
  }
...
export const PUT = async (req: NextRequest) => {
  ...
  try {
    ...
    if (file) {
      ...
      const existingResponsable = await prisma.responsableSante.findUnique({
        where: { id },
      });
    ...
    const updatedResponsable = await prisma.responsableSante.update({
      where: { id },
      data: {
        fullName: body?.fullName as string,
        poste: body?.poste as string,
