...
export const POST = async (req: NextRequest) => {
  ...
  try {
    const newPersonelRadio = await prisma.responsableHomonorie.create({
      data: {
        fullName: body?.fullName as string,
        poste: body?.poste as string,
        tel: body?.tel as string,
        homonorieId: +body?.homonorieId as any,
        thumbnail: "/uploads/homonorie/" + uniqueName,
      },
    });
...
export const GET = async (req: NextRequest) => {
  ...
  try {
    if (id) {
      // Recherche par ID
      const personelRadio = await prisma.responsableHomonorie.findUnique({
        where: { id: Number(id) },
      });
      ...
      // Recherche par nom
      const totalCount = await prisma.responsableHomonorie.count({
        where: {
          fullName: {
            contains: name,
      ...
      const personelRadios = await prisma.responsableHomonorie.findMany({
        where: {
          fullName: {
            contains: name,
            // mode: 'insensitive' // Décommentez ceci si vous voulez rendre la recherche insensible à la casse
...
export const DELETE = async (req: NextRequest) => {
  ...
  try {
    ...
    await prisma.responsableHomonorie.delete({
      where: { id: Number(id) },
    });
...
export const PUT = async (req: NextRequest) => {
  ...
  try {
    ...
    if (file) {
      ...
      const existingPersonelRadio =
        await prisma.responsableHomonorie.findUnique({
          where: { id: id },
        });
    ...
    const updatedPersonelRadio = await prisma.responsableHomonorie.update({
      where: { id: id },
      data: {
        fullName: body?.fullName as string,
        poste: body?.poste as string,
        tel: body?.tel as string,
        homonorieId: +body?.homonorieId as any,
        ...(file && { thumbnail: "/uploads/homonorie/" + uniqueName }),
      },
    });
